...
ConnectMiddleware.prototype.routeVerbGET = function(apiCore) {
  ...
  return function routeVerbGET(req, res, next) {
    ...
    var emitData = null,
      ...
      processorEvent = (routeObj.processor) ? routeObj.processor : routeObj.emit,
      crudEvent = routeObj.emit;

    emitData = _this.getRequestVariables(req, res, next);

    // console.log("--- running routeVerbGET ", processorEvent, crudEvent);

    // emit the processor event which generates a mongoose query 
    // object out of emitData and then emits crudEvent
    apiCore.intercom.emit( processorEvent, emitData, crudEvent );
