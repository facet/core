...
ConnectMiddleware.prototype.facetInitBuilder = function(apiCore) {
  ...
  return function facetInit(req, res, next) {
    ...
    apiCore.intercom.emit('facet:init:nodestack', nodeStack);
    next();
  }
};
    ...
    try {
      ...
      if( req.hasOwnProperty('query') ){
        ...
        if( req.query.hasOwnProperty('f') && req.query.f ){
          ...
          if( /\s*,\s*/.test(req.query.f) ) {
            var fields = req.query.f.split(/\s*,\s*/g);
          }
          // assume a space is used as delimiter
          else {
            var fields = req.query.f.split(/\s+/g);
          }

          this.apiCore.constants.STRIP_FIELDS.GET.forEach(function(field) {
