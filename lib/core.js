...
FacetCore.prototype.bindRoutes = function( router, routeOptions ) {
  // console.log("STARTING bindRoutes ", util.inspect(routeOptions, {depth:null}));
  // console.log("");
  ...
  if(routeOptions.hasOwnProperty('routes') && _.isArray(routeOptions.routes)){
    for( var i in routeOptions.routes ) {
      ...
      if( route.hasOwnProperty('extended') ) {
        ...
        if( _.isArray(route.extended) ) {
          ...
          for (var i = 0; i < extendObj.length; i++) {
            this[apiInstanceName].routerManifest.setRoutes(extendObj[i].routes);
          };
        }
        else if( route.hasOwnProperty('routeBase') && route.hasOwnProperty('routes') ) {
          route.routes = this._prepareExtendedRoutes(route.routeBase, route.routes);
          this[apiInstanceName].routerManifest.setRoutes(route.routes);
        }
        else {
          throw new Error('Extended routes must be defined as an object containing \'routeBase\' and \'bind\' keys or an array of such objects.')
        }
      }
      ...
      if( this.hasOwnProperty(apiInstanceName) ) {
        ...
        // TODO: errors should really be handled by a dedicated class
        throw new Error('No '+apiInstanceName+' key found on the current API class.');
      }
    } // end for( var i in routeOptions.routes )
  }
  // check for routeBase key, ie this is a single api class module
  else if(routeOptions.hasOwnProperty('routeBase')) {
    // set the route base
