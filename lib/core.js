  ...
  if( _.isFunction(successCb) ) {
    ...
    promise.then(function(data) {
      ...
      if( data === null ) {
        ...
        if(
          _this.hasOwnProperty('routerManifest')
          && _this.routerManifest.hasOwnProperty('manifest')
          && _this.routerManifest.manifest.hasOwnProperty('routeErrorMessages')
          && _this.routerManifest.manifest.routeErrorMessages.hasOwnProperty('find')
        ){
          // override the default findMessage
          findMessage = _this.routerManifest.manifest.routeErrorMessages.find;
        }
        _this.intercom.emit('facet:response:error', 404, findMessage);
      }
      else {
    ...
    function(err) {
      ...
      _this.intercom.emit('facet:response:error', status, errors);
    })
    .end();
  }
  else {
