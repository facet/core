...
FacetApiCore.prototype.setCommonAttributes = function ( options ) {
  ...
  this.doAccessCheck = options.doAccessCheck || false;
...
FacetApiCore.prototype.findOne = function(query, successCb, errorCb) {
  ...
  if(initialApiAuth === true || this.doAccessCheck === false) {
    this.checkingApiUser = false;
    var promise = queryBuilder.exec();
    var result = _this.respond( 'facet:response:' + this.routerManifest.manifest.apiEventType + ':data', promise, this.routerManifest.manifest.routeErrorMessages.findOne, successCb, errorCb );
    ...
    this.intercom.emit('facet:intercom:check:access', 
      attemptedEvent, 
      this.makeCheckAccessCb(queryBuilder, returnPromise, 'findOne(): Insufficient access', successCb, errorCb));
    return returnPromise;
  }
...
FacetApiCore.prototype.create = function(data, successCb, errorCb) {
  ...
  // do not access check the initial api access check
  if(this.doAccessCheck === false) {
    var result = this.respond( 'facet:response:' + this.routerManifest.manifest.apiEventType + ':data', promise, this.routerManifest.manifest.routeErrorMessages.findOne, successCb, errorCb );
    if( typeof result !== 'undefined' ) {
      return result;
    }
  }
