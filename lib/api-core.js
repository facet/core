...
FacetApiCore.prototype.findOne = function(query, successCb, errorCb) {
  ...
  if(initialApiAuth === true) {
    this.checkingApiUser = false;
    var promise = queryBuilder.exec();
    _this.respond( 'facet:response:' + this.routerManifest.manifest.apiEventType + ':data', promise, this.routerManifest.manifest.routeErrorMessages.findOne, successCb, errorCb );
...
FacetApiCore.prototype.update = function(query, successCb, errorCb) {
  ...
  var promise = this.Model.update(query.conditions, query.updates, query.options).exec();
  var result = this.respond('facet:response:user:update', promise, _this.routerManifest.manifest.routeErrorMessages.update, successCb, errorCb);
