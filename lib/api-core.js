...
FacetApiCore.prototype.find = function( query, successCb, errorCb ){
  ...
  if( !_.isUndefined(this._req.apiUser) ) {
    if( _.isEmpty(this._req.apiUser.container_id) ) {
      this.intercom.emit('facet:response:error', 400, this.routerManifest.manifest.routeErrorMessages.container);
      return Promise.reject(this.routerManifest.manifest.routeErrorMessages.container);
    }
...
FacetApiCore.prototype.create = function(data, successCb, errorCb) {
  ...
  if(!data.hasOwnProperty('container_id') && this.intercom.hasOwnProperty('container_id')){
    data['container_id'] = this.intercom.container_id;
  }

  if(!data.hasOwnProperty('app_id') && this.intercom.hasOwnProperty('app_id')){
    data['app_id'] = this.intercom.app_id;
  }
