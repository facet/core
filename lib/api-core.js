...
//   // this has been moved to FacetCore
// };
...
FacetApiCore.prototype.find = function( query, successCb, errorCb ){
  ...
  if( typeof query.fields === undefined || query.fields === null ) {
    ...
    var fields = this.cleanFields(query.fields);  
  }
 ...
 * @return   {string}
 */
FacetApiCore.prototype.cleanFields = function(fields) {
  // check if comma was used as delimiter
  if( fields ) {
    if( /\s*,\s*/.test(fields) ) {
      fields = fields.split(/\s*,\s*/g);
    }
    // assume a space is used as delimiter
    else {
      fields = fields.split(/\s+/g);
    }

    if( this.STRIP_FIELDS && _.isArray(this.STRIP_FIELDS.GET) ) {
      this.STRIP_FIELDS.GET.forEach(function(field) {
...
FacetApiCore.prototype.bindRouterManifest = function() {
  ...
  // now that all routes have been bound, lets also add
  // middleware to handle 404s
  this.router.use(this.middleware.notFound(this));
